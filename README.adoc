...
. Copy the application manifest  `manifest.yml` from your `cities-service/templates` directory to `cities-service`.  Minimally, you must set the name of the app, the amount of memory, the number of instances, and the path to the .jar file.
+
[source,bash]
----
